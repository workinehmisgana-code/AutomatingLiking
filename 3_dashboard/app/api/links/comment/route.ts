...
import { getActiveCommentProducts, pickFairProductForUrl } from '@/lib/db'
import { getFreshComments } from '@/lib/commentGen'
import { isProduct } from '@/lib/config'
...
export async function GET(req: NextRequest) {
  ...
  if (wanted) {
    ...
    if (active.length === 0) {
      return NextResponse.json({ error: 'No comment products are active' }, { status: 409 })
    }
  ...
  if (clean.length === 0) {
    return NextResponse.json({ error: `No comments stored for ${product}` }, { status: 409 })
  }
