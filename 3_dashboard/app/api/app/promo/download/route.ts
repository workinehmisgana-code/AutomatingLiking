...
import {
  ...
  getPromoDownloadInfo, getPromoTaskEnabled } from '@/lib/db'

export const dynamic = 'force-dynamic'

// POST { videoId } — record a promo download, enforcing the daily cap.
    ...
    if (!already) {
      ...
      if (todayCount >= PROMO_DOWNLOAD_DAILY_LIMIT) {
        ...
        return NextResponse.json(
          { error: `You've used today's download (${PROMO_DOWNLOAD_DAILY_LIMIT} per day). Come back tomorrow.`, nextResetMs },
          { status: 429 }
        )
      }
    }
